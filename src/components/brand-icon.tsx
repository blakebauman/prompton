...
type BrandIconProps = {
  ...
  path: string;
  className?: string;
  /** OpenAI (and some other) marks need evenodd for the glyph to read correctly. */
  evenodd?: boolean;
};

/** Monochrome brand mark from a Simple Icons–style path (currentColor). */
export function BrandIcon({ title, path, className, evenodd }: BrandIconProps) {
  return (
    <svg
      role="img"
      viewBox="0 0 24 24"
...
export function DialectIcon({
  ...
  switch (dialect) {
    ...
    case "mysql":
      return (
        <BrandIcon title={siMysql.title} path={siMysql.path} className={className} />
      );
    case "sqlite":
      return (
        <BrandIcon
...
/**
 * OpenAI glyph from @lobehub/icons-static-svg (MIT). Simple Icons no longer
 * ships the OpenAI brand mark.
 */
const OPENAI_PATH =
